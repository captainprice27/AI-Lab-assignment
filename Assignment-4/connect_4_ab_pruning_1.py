...
def alpha_beta_pruning(game, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        ...
        for col in valid_locations:
            ...
            new_score = alpha_beta_pruning(game_copy, depth-1, alpha, beta, False)[1]
            if new_score > value:
                value = new_score
                column = col
        ...
        for col in valid_locations:
            ...
            new_score = alpha_beta_pruning(game_copy, depth-1, alpha, beta, True)[1]
            if new_score < value:
                value = new_score
                column = col
            beta = min(beta, value)
...
def play_game_alpha_beta_visual():
    """Play a visual game of Connect-4 against the AI using Alpha-Beta Pruning."""
    # Setup pygame display
    screen = pygame.display.set_mode(size)
    pygame.display.set_caption('Connect 4 - Alpha-Beta Pruning')
    ...
    title_text = title_font.render("Connect 4 with Alpha-Beta", True, WHITE)
    ...
    while not game_over:
        ...
        if turn == 1 and not game_over:
            ...
            # Get AI's move using Alpha-Beta Pruning
            ai_start_time = time.time()
            col, score = alpha_beta_pruning(game, 5, -float('inf'), float('inf'), True)  # Depth of 5 (can be higher than Minimax)
            ai_end_time = time.time()
            decision_time = ai_end_time - ai_start_time
            ...
            if game.is_valid_location(col):
                ...
                # Show AI's decision time
                pygame.draw.rect(screen, BLACK, (0, 0, width, SQUARESIZE))
                decision_font = pygame.font.SysFont("monospace", 20)
                decision_text = decision_font.render(f"AI chose column {col} in {decision_time:.3f}s", True, WHITE)
                screen.blit(decision_text, (width//2 - decision_text.get_width()//2, SQUARESIZE//2 - decision_text.get_height()//2))
                pygame.display.update()
                pygame.time.wait(1000)  # Show the decision time for 1 second
